...
CREATE TABLE "Description" ("poiID" INTEGER NOT NULL, "language" TEXT NOT NULL, "age" TEXT NOT NULL , "description" TEXT NOT NULL, FOREIGN KEY (poiID) REFERENCES POI(poiID) ON DELETE CASCADE, FOREIGN KEY ("language") REFERENCES Language("language") ON DELETE CASCADE );
...
DROP TABLE IF EXISTS "ItineraryTitles";
...
DROP TABLE IF EXISTS "LocationGuideLines";
CREATE TABLE "LocationGuideLines" ("poiID" INTEGER NOT NULL ,"language" TEXT NOT NULL ,"locationGuideLines" TEXT NOT NULL, FOREIGN KEY (poiID) REFERENCES POI(poiID) ON DELETE CASCADE, FOREIGN KEY (language) REFERENCES Language(language) ON DELETE CASCADE );
...
CREATE TABLE "POI" ("poiID" INTEGER PRIMARY KEY  NOT NULL  UNIQUE , "city" TEXT NOT NULL , "locationID" INTEGER NOT NULL, "address" TEXT NOT NULL, FOREIGN KEY (locationID) REFERENCES Location(locationID) ON DELETE CASCADE
);
INSERT INTO "POI" VALUES(1,'Louvain-La-Neuve',1,'Rue du Labrador 26, 
1348 Louvain-La-Neuve');
INSERT INTO "POI" VALUES(2,'Louvain-La-Neuve',2,'Place de l''Accueil, 
1348 Louvain-la-Neuve');
INSERT INTO "POI" VALUES(3,'Louvain-La-Neuve',3,'Grand-Place 55, 
1348 Louvain-la-Neuve');
INSERT INTO "POI" VALUES(4,'Louvain-La-Neuve',4,'Place Blaise Pascal, 1 bte  L3.03.01, 
1348 Louvain-la-Neuve');
INSERT INTO "POI" VALUES(5,'Louvain-La-Neuve',5,'Place des Brabançons 1A,
1348 Louvain-La-Neuve');
INSERT INTO "POI" VALUES(6,'Louvain-La-Neuve',6,'Place Sainte-Barbe,
1348 Louvain-La-Neuve');
INSERT INTO "POI" VALUES(7,'Louvain-La-Neuve',7,'Place des Sciences, 
1348 Louvain-la-Neuve');
INSERT INTO "POI" VALUES(8,'Louvain-La-Neuve',8,'Grand-Place, 
1348 Louvain-la-Neuve');
INSERT INTO "POI" VALUES(9,'Louvain-La-Neuve',9,'Place de l''Université, 
1348 Louvain-La-Neuve');
INSERT INTO "POI" VALUES(10,'Louvain-La-Neuve',10,'Place des Wallons, 
1348 Louvain-La-Neuve');
INSERT INTO "POI" VALUES(11,'Louvain-La-Neuve',11,'Rêverie du Promeneur Solitaire, 
1348 Louvain-La-Neuve');
...
DROP TABLE IF EXISTS "POIItinerary";
...
DROP TABLE IF EXISTS "POITitles";
CREATE TABLE "POITitles" ("poiID" INTEGER NOT NULL ,"language" TEXT NOT NULL ,"title" TEXT NOT NULL, FOREIGN KEY (poiID) REFERENCES POI(poiID) ON DELETE CASCADE, FOREIGN KEY (language) REFERENCES Language(language) ON DELETE CASCADE);
...
CREATE TABLE "User" ("userName" TEXT PRIMARY KEY  NOT NULL ,"age" varchar(5) NOT NULL check("age" in('adult','kid')));
INSERT INTO "User" VALUES('Maxime','adult');
INSERT INTO "User" VALUES('Pierre','kid');
INSERT INTO "User" VALUES('Hervé','adult');
DROP TABLE IF EXISTS "UserCategory";
CREATE TABLE "UserCategory" ("categoryID" INTEGER NOT NULL, "userName" TEXT NOT NULL, FOREIGN KEY (categoryID) REFERENCES CategoryTitles(categoryID) ON DELETE CASCADE, FOREIGN KEY (userName) REFERENCES User(userName) ON DELETE CASCADE, FOREIGN KEY (categoryID) REFERENCES CategoryTitles(categoryID) ON DELETE CASCADE
);
...
CREATE TABLE "UserItinerary" ("itineraryID" INTEGER NOT NULL,"userName" TEXT NOT NULL,  FOREIGN KEY (itineraryID) REFERENCES ItineraryTitles(itineraryID) ON DELETE CASCADE, FOREIGN KEY (userName) REFERENCES User(userName) ON DELETE CASCADE,  FOREIGN KEY (itineraryID) REFERENCES ItineraryTitles(itineraryID) ON DELETE CASCADE);
INSERT INTO "UserItinerary" VALUES(1,'Maxime');
CREATE TRIGGER on_delete AFTER DELETE ON POI BEGIN
  DELETE FROM POICategory WHERE poiID = old.poiID;
  DELETE FROM POITitles WHERE poiID = old.poiID;
DELETE FROM POIItinerary WHERE poiID = old.poiID;
DELETE FROM LocationGuideLines WHERE poiID = old.poiID;
DELETE FROM Image WHERE poiID = old.poiID;
DELETE FROM Description WHERE poiID = old.poiID;
END;
CREATE TRIGGER on_deleteCategoryID AFTER DELETE ON CategoryTitles BEGIN
  DELETE FROM UserCategory WHERE categoryID = old.categoryID;
DELETE FROM POICategory WHERE categoryID = old.categoryID;
END;
CREATE TRIGGER on_deleteItineraryID AFTER DELETE ON ItineraryTitles BEGIN
  DELETE FROM UserItinerary WHERE itineraryID = old.itineraryID;
DELETE FROM POIItinerary WHERE itineraryID = old.itineraryID;
END;
CREATE TRIGGER on_deleteLanguage AFTER DELETE ON Language BEGIN
  DELETE FROM CategoryTitles WHERE language = old.language;
DELETE FROM Description WHERE language = old.language;
DELETE FROM ItineraryTitles WHERE language = old.language;
DELETE FROM CategoryTitles WHERE language = old.language;
DELETE FROM LocationGuideLines WHERE language = old.language;
DELETE FROM POITitles WHERE language = old.language;
END;
CREATE TRIGGER on_deleteLocationID AFTER DELETE ON Location BEGIN
  DELETE FROM POI WHERE locationID = old.locationID;
END;
CREATE TRIGGER on_deleteUserName AFTER DELETE ON User BEGIN
  DELETE FROM UserCategory WHERE userName = old.userName;
DELETE FROM UserItinerary WHERE userName = old.userName;
DELETE FROM Langage WHERE userName = old.userName;
END;
CREATE TRIGGER update_user AFTER UPDATE ON User BEGIN
  UPDATE UserCategory SET userName = new.userName WHERE userName = old.userName;
  UPDATE Language SET userName = new.userName WHERE userName = old.userName;
END;
